...
void classAndAccessSpecifiers(){
    // When we use this inside a class , it is actually a pointer
...
void Constructor(){
    ...
    // Concept of Constructor Overloading: We can create two constructors and depending upon the arguments passed ,  they will be called .

    // Destructor is called when the declared object is destroyed. They are specifically useful if you have any dynamic memory allocation inside the constructor(so you can erase it). A Destructor function neither have a return type nor any paramteres. It is declared as ~<name_of_class>
